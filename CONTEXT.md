...
本論文的核心 feature innovation。基於 `meter_reading` 與 `t-s` 時刻讀數的差值 `X(t) - X(t-s)` 或比值 `(X(t)+1)/(X(t-s)+1)` 衍生的特徵,用以捕捉時間序列的變化幅度。論文 §2.2 描述 32 個(8 個 shift 值 × 2 方向 × 差值/比值兩類),但 Table 3 總計 169 features 暗示還有約 80 個未說明的 value-change features。用以偵測 point anomaly(急遽變化)和 sequential anomaly(零變化 / flatline)。
