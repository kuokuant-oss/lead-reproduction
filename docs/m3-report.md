...
| Regime | Feature availability | Features | Intended interpretation |
|---|---|---:|---|
...
| Stage | Split | Regime | Features | AUC | Interpretation |
|---|---|---|---:|---:|---|
| M3.1 baseline | 80/20 | Offline | 17 | 0.9562 | Time, metadata, weather baseline |
| M3.2 value-change | 80/20 | Offline | 137 | 0.9920 | Main feature-engineering jump |
| M3.2 value-change | 80/20 | Causal | 77 | 0.9908 | Past-only value-change remains strong |
| M3.2 value-change | 50/50 | Offline | 137 | 0.9914 | Final split, single LightGBM |
| M3.2 value-change | 50/50 | Causal | 77 | 0.9903 | Final split, past-only LightGBM |
| M3.3 buds-lab alignment | 80/20 | Offline | 170 | 0.9913 | No robust AUC lift |
| M3.4 ensemble | 80/20 | Offline | 137 | 0.9928 | Small ensemble lift |
| M3.4 ensemble | 50/50 | Offline | 137 | **0.9921** | Final offline result |
| M3.4 ensemble | 50/50 | Causal | 77 | **0.9911** | Final causal result |
| M3.5 post-processing | 80/20 | Offline | 137 | 0.9927 | Null result; rules do not transfer |

## 3.2 Feature Engineering

M3.1 uses 17 baseline features: time features, building metadata, meter type,
meter reading, and weather. Adding 120 value-change features in M3.2 is the main
performance jump:
...
The value-change features use the same shift family as M2:
`-24..-1`, `1..24`, `-168..-48 step 24`, and `48..168 step 24`, with both
difference and ratio forms. In pandas, positive shifts use past readings and
negative shifts use future readings.
...
## 4.1 Leakage and Split Checks

No evidence of train/validation building leakage or target-encoder leakage was
found. Future-shift features are non-causal, but they appear symmetric with
past-shift features in this offline batch-labeling setup.

| Check | Result | Interpretation |
|---|---|---|
| Building overlap | 0 in all reported splits | Validation buildings are held out by `building_id`. |
| Past-only vs future-only | Past `0.9908`, future `0.9908`, full `0.9920` | Future shifts are non-causal but not uniquely responsible for the AUC. |
| Label shuffle, M3.2 seed 42 | 0.5669 | Elevated over random; treated as residual structure/base-rate signal. |
| Label shuffle, M3.5 seeds 42/123/999 | 0.5669 / 0.5669 / 0.4232, mean 0.5190 | Shuffle signal is unstable and far below the real-label result. |
| Remove meter features | AUC drops to 0.8160 | Meter reading and value-change features carry the main anomaly signal. |
| M3.3 target encoder ablation | Removing `gte_site_meter_anomaly` does not reduce shuffle AUC | Target encoding is not the source of the elevated shuffle result. |

The appropriate conclusion is cautious: the checks do not show evidence of
split leakage, but label-shuffle results show that metadata/base-rate structure
...
|---|---:|---|
| Site-held-out ensemble AUC (`site_id % 5 == 4`) | 0.9774 | Cross-site validation is materially harder than building-held-out validation. |
| Per-meter AUC: electricity / chilled water / steam / hot water | 0.9991 / 0.9888 / 0.9553 / 0.9863 | Steam is the weakest meter slice. |
| Buildings with missing hours inside observed range | 945/1449 (65.2%) | Row-offset value-change shifts are approximations across timestamp gaps. |

The value-change implementation uses `groupby().shift()`, so shifts are
row-offset features rather than exact `timestamp + timedelta` joins. This is a
documented limitation for interpreting long-range shifts.

## 4.3 Primary-use Slices

The following AUCs are computed by joining
`data/processed/m3_5_val_predictions.csv.gz` with
`data/raw/m3/building_metadata.csv`. The full machine-readable table is archived
in `docs/m3-primary-use-auc.json`.
...
Several primary-use categories have very few validation buildings, so those
slices should not be treated as stable building-type conclusions. Among the
large slices, Education is the lowest at `0.9894`, while Office is `0.9941`.
...
|---|---|---:|
| Electricity | Easiest / best predicted | 0.9991 |
| Chilled water | Harder than electricity | 0.9888 |
| Steam | Harder than electricity | 0.9553 |
| Hot water | Hardest in III2, about 40% good fit | 0.9863 |
