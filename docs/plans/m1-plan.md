...
| 產出 | 說明 |
|------|------|
| `docs/reference/paper-notes.md` | 論文全文精讀筆記,涵蓋 pipeline 七階段、公式、關鍵數字 |
| `docs/reference/unknowns.md` | 6 個論文未說明的 B 類 unknown,附脈絡與重要性說明 |
...
- [ ] 原本未知的 ~80 個特徵來源已確認
- [ ] `docs/reference/unknowns.md` Unknown #1 標記為 resolved,附解答摘要
- [ ] `docs/reference/paper-notes.md` Feature Engineering 章節補充更新(若有新發現)
...
- [ ] `docs/reference/unknowns.md` Unknowns #2、#4、#5 標記為 resolved
- [ ] 如有新的設計決策,補充 ADR
...
- [ ] `docs/reference/unknowns.md` Unknowns #3、#6 標記為 resolved
...
- 四個 GBDT 模型超參數提取自 Modeling notebook Cells 8–11
- 核心發現:全部使用庫預設值;論文「hyperparameter tuning」無對應實作
- Train AUC 0.9999 來自特徵判別力 + balanced downsampling,非特殊超參數
- Rule 2 邊界已於 B.1(Unknown #3)解決,無需額外處理
- Unknown #6 → resolved;文件化至 feature-engineering-rules.md + unknowns.md
...
- 確認訓練資料的 schema 與 `docs/reference/paper-notes.md` 描述一致(欄位名稱、筆數大致符合)
...
| Issue 2: CV / downsampling / target encoding | #5 | partially resolved — 剩下需 M2 實驗驗證,已移至 M2 |
| Issue 3: hyperparameters + post-processing | #6 | ✅ done |
| Issue 4: dev environment | #7 | ✅ done |
...
| Unknown | 狀態 | 備註 |
|---------|------|------|
| #1 Feature count 169 | resolved ✓ | 完整 169 欄組成確認 |
| #2 Validation split 細節 | partially-resolved | split 機制確認;確切建築數 deferred to M2 |
| #3 Rule 2 start/end 邊界 | resolved ✓ | 兩個 IF 條件確認 |
| #4 Downsampling 作用域/seed | partially-resolved | seed 10+20 確認;語義釐清 deferred to M2 |
| #5 Target encoding leakage | partially-resolved | 機制確認;leakage 量化 deferred to M2 |
| #6 GBDT 超參數 | resolved ✓ | 全部使用庫預設值 |
| #7 LEAD CSV 上游 | resolved ✓ | `02_preprocess_data.py` 確認 |

**結論**: 4 個 resolved + 3 個 partially-resolved(全部有足夠的實作決策依據)。#2/#4/#5 的剩餘問題不阻塞 M2 開始,移至 M2 milestone 以實驗驗證。
...
| 風險 | 說明 | 緩解方式 |
|------|------|---------|
| **原始碼不完整** | `buds-lab/LEAD-1st-solution` 的 notebooks 可能有硬編碼路徑、中途輸出或未更新的變數名稱,使部分 unknowns 難以直接從 code 回答 | 若確認後仍有歧義,在 `docs/reference/unknowns.md` 標記為「partially resolved」並在 ADR 中記錄假設值 |
...
| **M1 範圍蔓延** | 讀完原始碼後可能發現論文還有其他未記載的設計決策 | 新發現的 unknowns 若屬 M2 必要,立即加 ADR;若屬可選優化,放入 `docs/reference/unknowns.md` 標記 C 類 |
...
- `docs/reference/feature-engineering-rules.md` — complete feature + model rules
- `docs/reference/unknowns.md` — all 7 unknowns with current status
