...
**結果（M2.4）**：6 天累積工作，單次提交，Private Score 0.98616，gap 1.68%。
...
*Paper score 理解偏移*：Claude Code 在 commit `cf1097b` 寫「paper = Public Score
(confirmed)」。後來透過比對原作者 Kaggle profile 實際分數，發現 paper 的 0.9866
是 Public Score（Private gap 1.68%），不是 Private Score。Lesson #7 記錄了這次
校準（「Verify paper performance on leaderboard separately」）。
