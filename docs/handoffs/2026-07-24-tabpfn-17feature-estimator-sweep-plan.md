...
### 3.3.1 n=8 實測：estimator 不是線性成本（推翻 §4 的原估）
...
1. 原本「estimator 只影響 <2%」被推翻：實際影響橫跨 −0.07 到 +0.62，差兩個數量級。
2. 「提高 estimator 普遍有幫助」也被推翻：Site 2 的 PR-AUC 隨 n **單調下滑**（0.2807 → 0.2262 → 0.2101），
