...
被當成「特徵不足」證據的橘 vs 紫落差，**同時混了特徵數（17→137）與 estimator 數（1→8）兩個變因**。
137-feature handoff 的歸因結論目前是被 confound 的。

本次把橘線也換成 n=8，兩條 TabPFN 線就只差在特徵集合。

**不預期橘線一定變好。** estimator sweep 已證明 n 的效果沒有單一方向 —— Site 1 明顯改善、
Site 2 的 ROC 與 PR **同時退步**、Site 3 的 PR-AUC 從 0.167 衝到 0.783 而 ROC 幾乎不動。
pooled 方向要跑完才知道，文件與圖說一律不預告方向。
...
- 12 個 shard 的 `raw_index_sha256` 與 `label_sha256` **全部等於 137 線同名 shard 的值**（12/12）。
  也就是說兩條線的每個 shard 覆蓋**完全相同的列、相同順序** —— 這比「兩邊都宣稱用同一個計畫」強得多。
...
批次幾何與 137 線逐位相同，且已用 raw_index/label digest 證明兩條線覆蓋完全相同的列 ——
這是「跑完之後兩條 TabPFN 曲線可以並排」的硬證據，不是宣稱。

本次的價值在**移除橘 vs 紫之間的 estimator confound**，不在「換個更好的數字」；
跑完之後 137-feature handoff 的歸因才有資格被確認或推翻。
