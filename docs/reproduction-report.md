...
**Lesson #7 — Reproduction metric source 必須對齊**:Paper §3 報告 0.9866,
最初以為是 Public Score。比對原作者實際 Kaggle 分數後確認:0.9866 ≈ Private
rounded(原作者 Private = 0.9866,Public = 0.9734;我們 Private 0.9862 vs Public
0.9698)。Paper 與 Public 差 0.0132,與 Private 差 0.0005——後者在 noise floor 範圍
內。其他 reproducer 看 paper Table 2 應將 0.9866 視為 Private。

**為什麼這個會踩雷**:Kaggle 比賽期間,Public Score 是公開可見的,
Private Score 比賽結束才揭曉。多數 paper 寫 "test AUC" 或 "leaderboard score" 時,
讀者直覺會聯想到 Public(因為比賽期間 Public 更顯眼)。但 Kaggle 競賽正確的
"final score" 是 Private——這是 paper 應該對齊的指標。Fu et al. paper 未明確區分,
我們需要跟原作者比對才能確認。對其他 reproducer 的含義:看任何 Kaggle paper 的
"test AUC" 前,先去比賽 leaderboard 找原作者實際分數確認。
...
## 4.3 Paper 沒覆蓋的 findings

### a. Paper 內部矛盾類

**a1. §3 text vs Fig 3 confusion matrix 不一致**

Paper §3 text 寫 precision 98.7% / recall 81.9%。但 Fig 3 以百分比顯示的
confusion matrix(TN 96.3%, FP 1.4%, FN 0.2%, TP 2.0%)back-calculate 出
precision=58.8% / recall=90.9%——兩者無法同時為真。我們的數字(98.7% / 81.2%)
完全支持 §3 text。Fig 3 的百分比是相對於 total prediction 的比例,不是
standard confusion matrix 的 row 正規化,讀者直接代入 precision/recall 公式
會算出錯誤結果。

### b. Paper 沒揭露的 implicit design

**b1. Rule 2a 的 building_id filter**

Paper §2.4 只說 "start points of time series → set prediction to 0"。代碼實際是:
...
105≤id≤145 之間的建築不套用此 rule(推測這些建築在 year day 1 有合法 anomaly)。
Paper §2.4 完全未提 building_id filter(unknown #15)。Rule 2b(end-of-year)無此過濾。

**b2. Val/test post-processing asymmetry**

Downsampling 在 CV split 前執行,val 的 `dayofyear==1` rows 被 downsampling
取樣移除。Rule 2a 在 val 觸發 **0 rows**,在 test 觸發 **192 rows**。Post-processing
的效果只能在 test set 上看到。Paper §2.4 描述如通用 rule,但 val 端完全看不到
ΔAUC 貢獻。

**b3. X_all dual-path submission 流程**

Paper Fig 1 只顯示一條 pipeline。實際上:
- **Evaluation**: `scaler.fit_transform(X_train)` → val 評估
- **Submission**: `scaler.fit_transform(df_eq[features])` → 全部 downsampled data
  refit,再 predict test set

這個 dual-path 在 buds-lab Modeling notebook Cell 13 才能看到(`X_all` 在 notebook
從未被明確定義——需要從上下文推斷)(unknown #16)。

### c. 環境/實作層級的 reproduction 陷阱

**c1. sklearn 1.4+ KMeans `n_init` default 靜默變更**

2022 年版本:`n_init='auto'` with k-means++ = 10。2024+ 改為 = 1。
不明確設定 `n_init=10` → ARI=0.503(錯誤分群)。ClusterNo 是 169 features
的一部分,錯誤的 cluster labels 直接影響 AUC。

**c2. CatBoost 1000 iterations vs 其他 100**

LGB / XGB / HistGBT 都是 100 iterations(library default),CatBoost 是 1000
(也是 library default,但比其他高 10×)。Paper §2.3.3 說 "will be considered
for hyperparameter tuning",但代碼用的是 defaults。這個不對稱沒有 paper
justification(unknown #6)。

**c3. Noise floor 實測 vs 假設**

最初假設 ±0.002。加 `random_state=42` 後兩次 run bit-for-bit identical,
noise floor 收斂為 ±0.0005。這影響所有 ablation 的顯著性判斷:ΔAUC > 0.0005
才算 confirmed(之前認為 ±0.002 以下的 ΔAUC 都可能是噪音)。

### d. M2.5 ablation findings

3 個 in-notebook ablation + 3 Kaggle submission(總計 1+3 = 4 submissions)量化
unknown #5、#10、#15。

**a. Unknown #5: gte_* 是 valid target encoding**
- 16 個 gte_* features 提供 ~0.001 val ΔAUC 貢獻
- Kaggle Private 量化 (Ablation A 含 ensemble effect): -0.004
- 結論:不是 harmful leakage,paper §2.2.4 未明列為 main contributor 是 underplay

**b. Unknown #10 candidate 1: 在我們 pipeline 上 raw NaN > per-bldg mean**
- 在我們的 reproduction pipeline 上:per-bldg mean impute val -0.0058, Kaggle Private -0.0128
- **重要 caveat**:這只反映我們 pipeline 的 component interaction,**不能延伸到 paper 的 pipeline**
- 原作者完整 pipeline (含 mean imputation) Private 0.98661 仍比我們 0.98616 高
- 結論:Paper 整體設計 well-tuned,component swap 不一定能改善 reproduction

**c. Unknown #15: Rule 2a building_id filter 是 building-level 精細設計**
- Filter 保護 14 棟 buildings (105≤id≤145) 的 dayofyear==1
- Dataset-level Kaggle Private ΔAUC = -0.0001 (≈ noise floor)
- 結論:Paper §2.4 未揭露 filter 邏輯合理 — dataset-level effect 太小不值得描述
...
| 7 | M2.4 | Reproduction 開始就要確認 paper metric 對應哪個 leaderboard;paper 0.9866 ≈ Private |
| 8 | M2.5 | **Component interaction matters**:Ablation 揭露在我們的 pipeline 內 swap component 的 effect,但**不能 generalize 為 paper 設計的全局 effect**。Paper 整體設計經過 tuning,各 component 之間有 interaction。Reproducer 應對齊完整 pipeline,而不是孤立看單一 component。 |
