...
結果：6 天累積 → **單次提交** → **Private Score 0.98616，gap 0.05%**。
...
| `lag_value_{n}` diff columns (60 shifts; category key `lag_value_diff`) | 60 |
...
(錯誤分群);明確設定後，對外部 buds-lab ClusterNo reference labels 測得
ARI=**1.0**,406/406 buildings 完全對齊。Reference labels 不是 committed
repo artifact；來源為 buds-lab Feature generator Cells 8-10 對應的 ClusterNo
labels,見 `docs/handoffs/2026-05-26-m22a-completed.md`。
...
Post-processing 在 val 幾乎沒有效果(ΔAUC = +0.0004; noise floor ±0.0005)。
...
Recall caveat: reproduction recall `81.2%` is measured on the 50:50 downsampled
validation set, while paper §3 recall `81.9%` is reported on the real
distribution. The `Δ 0.7%` is therefore indicative only, not a like-for-like
comparison.
...
Private gap 0.05% / 0.0004；noise floor ±0.0005 見上方定義。
...
Unknown #10 candidate 1:paper imputation 在本 reproduction pipeline 上 swap method (raw NaN → per-bldg mean) 降 0.0128；interpretation 見 component-interaction caveat (0.98661 vs 0.98616)。

**Ablation C (Rule 2a blanket, no building_id filter)**:
ΔPrivate = -0.0001。Rule 2a 的 building_id filter (id>145 OR <105; unknown #15, Rule 2a ΔAUC -0.0001) 保護 14 棟 buildings 的 dayofyear==1
...
Val pipeline downsampling 對某些 design 選擇的 sensitivity 較低(distribution shift)；component interpretation 見 component-interaction caveat。
...
| Recall | 81.2% | 81.9% (§3 text) | Δ 0.7%; indicative only because reproduction uses 50:50 downsampled validation while paper §3 uses the real distribution | M2.4 Phase 1 |
...
filter，Kaggle Private ΔAUC = -0.0001（noise floor ±0.0005 見上方定義）。觀察：filter
...
| B: per-bldg mean impute | 取代 raw NaN | -0.0128 | 見 component-interaction caveat |
| C: Rule 2a blanket（不 filter） | 14 棟 buildings 不保護 | -0.0001 | dataset-level effect small |
...
| M2.4 Phase 1: + post-proc | 0.9834 | — | — | ΔAUC +0.0004 |
...
| Val AUC | 0.9830 | — | — | 與 leaderboard gap 約 1% 內 |

Public AUC 0.9698、Val AUC 0.9830、Private AUC 0.98616 三者的差距皆在約
1% 以內，符合 §2.3.1 描述的 validation / leaderboard gap 範圍，也支持
building-based CV split 作為本 reproduction 的內部驗證設計。

Paper Table 2 寫的 0.9866 與復現 Private 0.98616 差 0.0004,與原作者 Private 0.98661
差 0.0005——±0.0005 noise floor 見上方定義。**Paper Table 2 報告的是 Private
(rounded),不是 Public**。
...
**6-7 天累積 → 單次 baseline 提交 + 3 ablation Kaggle 驗證 → Private gap 0.05%**。
...
**為什麼這個 framing 重要**:Private gap 0.0005；所有設計決策都記錄在
unknowns、ADR、commits 中。One-shot/no-probing 原則見 verification section。
...
| 7 | M2.5 | **Component interaction matters**:Ablation interpretation 見 component-interaction caveat。 |

## 5.5 M2 Exit Criteria 達標情況

| Criterion | 狀態 |
|-----------|------|
| LightGBM baseline 與 paper Fig 4 gap < 5% | ✅ 0.8952 vs 0.9311 (gap 3.86%) |
...
| Unknown #10 candidate 1 (impute_nulls) | ✅ Quantified; see component-interaction caveat |
