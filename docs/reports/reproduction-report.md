...
其中 120 個是以正負 shift 計算的差值與比值特徵；Val AUC 從 0.9311 跳到
0.9849,提升 +5.8%。Paper Table 3 中未處理 class imbalance 的解法(rank 3–5)
全部低於 0.90；0.93+ 來自 value-change feature engineering。4-model ensemble
再加 +0.21%。
...
Confusion matrix at threshold=0.5:precision 98.7%,recall 81.2%;paper §3 text
reports precision 98.7% / recall 81.9%。
...
Unknown #10 candidate 1:paper imputation 在本 reproduction pipeline 上 swap method (raw NaN → per-bldg mean) 降 0.0128；pipeline-local result。
...
| Recall | 81.2% | 81.9% (§3 text) | Δ 0.7%; indicative only | M2.4 Phase 1 |
...
filter，Kaggle Private ΔAUC = -0.0001。觀察：filter
...
| B: per-bldg mean impute | 取代 raw NaN | -0.0128 | pipeline-local |
...
差 0.0005。**Paper Table 2 報告的是 Private
(rounded),不是 Public**。
...
Recall 81.2% is measured on the 50:50 downsampled validation set, while paper §3
recall 81.9% is on the real distribution, so Δ 0.7% is indicative only. M2.5
ablation effects are pipeline-local; the full buds-lab pipeline reports
0.98661 vs this reproduction's 0.98616, and tiny deltas are judged against
noise floor ±0.0005.
...
| 7 | M2.5 | **Component interaction matters**:M2.5 ablations are pipeline-local。 |
...
| Unknown #10 candidate 1 (impute_nulls) | ✅ Quantified; pipeline-local |
