"""Render the provisional V5 fixed-50K building-count PR-AUC figure.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=here / "m5_exp_b_building_count_meter_pr_auc_v5_fixed50k_provisional.png",
    )
    parser.add_argument(
        "--summary",
        type=Path,
        default=here / "m5_exp_b_building_count_meter_pr_auc_v5_fixed50k_provisional_data.json",
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=here / ".m5_v5_fixed50k_plot_cache",
    )
    parser.add_argument(
        "--final",
        action="store_true",
...
def load_or_compute_cell_metrics(directory: Path, model: str, cache_dir: Path) -> dict[str, Any]:
    ...
    for meter_id, meter_name, _ in METERS:
        ...
        meter_pr_auc[meter_name] = float(average_precision_score(y, scores[keep]))
                ...
                seed_records.append(
                    {
                        ...
                        "meter": meter_name,
                        "ensemble": float(
                            np.mean([per_row[row]["ensemble"]["meter_pr_auc"][meter_name] for row in row_seeds])
                        ),
                        "tabpfn": float(
                            np.mean([per_row[row]["tabpfn"]["meter_pr_auc"][meter_name] for row in row_seeds])
                        ),
                    }
                )
                ...
                seed_records.append(
                    {
                        ...
                        "ensemble": float(
                            np.mean(
                                [
                                    row_metrics[row_seed]["meter_pr_auc"][
                                        meter_name
                                    ]
...
def build_k725_seed_records(
    ...
    for row_seed in K725_ROW_SEEDS:
        ...
        for meter_id, meter_name, _ in METERS:
            tabpfn_value = float(summary["per_seed_metrics"][str(row_seed)]["meters"][str(meter_id)]["pr_auc"])
            if row_seed < 2:
                computed = float(tabpfn["meter_pr_auc"][meter_name])
            ...
            seed_records.append(
                {
                    ...
                    "row_seed": row_seed,
                    "meter": meter_name,
                    "ensemble": float(tree["meter_pr_auc"][meter_name]),
                    "tabpfn": tabpfn_value,
                }
            )
    return cell_records, seed_records
...
def render(
    ...
    for index, (ax, (_, meter_name, meter_label)) in enumerate(zip(axes, METERS, strict=True)):
        ...
        for model in MODELS:
            ...
            y = [float(row["mean_pr_auc"]) for row in rows]
            yerr = np.asarray(
        ...
        ax.minorticks_off()
        # Lowest tick is 0.40; the axis extends slightly below it so that tick is
        # not flush with the frame and the lowest error bar (0.4113) clears it.
        ax.set_ylim(0.38, 1.01)
        ax.set_yticks(np.arange(0.40, 1.01, 0.10))
        ...
        if index == 0:
            ax.set_ylabel("PR-AUC", fontsize=9.2, color=SECONDARY, labelpad=7)

    fig.suptitle(
        "Experiment B — meter-level PR-AUC across building count",
        x=0.065,
        y=0.972,
        ha="left",
        fontsize=15,
