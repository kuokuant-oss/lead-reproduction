...
> 本文為 [`docs/reports/m5-foundation-vs-gbdt.md`](./m5-foundation-vs-gbdt.md) 的
> 正體中文翻譯。所有數字、模型名、欄位/識別碼、檔案路徑、連結與 issue 編號均與
> 英文版逐項一致；英文版為 provenance 原檔。
...
（commit `8f4373b`，產生於 2026-06-26 UTC）。
...
為 `0.9928`。TabPFN 在 10k context 下**追平該調校後的 ensemble**，而
single-GBDT-at-10k 的 baseline 則略低於它。所以這裡 TabPFN 並不是贏在打敗一個
弱模型 —— 它達到了調校後 ensemble 的頭條水準 —— 但代價是巨大的推論延遲，且
調校後的 GBDT line 並未被取而代之。
...
**誠實解讀。** 在**真正跨站**的模型中（只用 source sites 訓練），TabPFN-in-context
在 ROC-AUC（+0.0035）與 F1 上勝過 GBDT-retrain，但 GBDT-retrain 在 PR-AUC
上勝出（+0.010）—— 這是一個真實的勝負互見，而非 TabPFN 的全面碾壓。
**GBDT-transfer-without-retrain** 那一列有最高的 ROC-AUC（0.9882）與 PR-AUC
（0.9023），**但它是較容易的設定**：該模型是在 `80_20_mod5` building split 上
訓練的，其 source buildings 橫跨*所有* sites —— 包含 held-out sites 裡的其他
buildings。它因此帶有真正跨站模型所缺乏的站別熟悉度（weather regime、site mix）。
它回答的是「一個已部署的 all-sites 模型，能否泛化到*已知* sites 中的新
*buildings*？」，而不是「一個模型能否轉移到*未見過的 sites*？」這樣解讀下，它
並不構成對 TabPFN 轉移能力的反證。
...
+ **推論延遲**：GBDT 次秒級評分；TabPFN 的 `predict_proba` 對 4,000 rows 需約
  25 s（~6.3 ms/row）。就現況而言不適用於低延遲的 real-time FDD。這是一個離線
  benchmark，**不是** real-time FDD 的保證；任何 real-time 宣稱仍需依 ADR 0007
  與 ADR 0011 使用 `PAST_SHIFTS`-only 的 causal features。
...
cross-site** 的情境，在那裡它帶來真實價值（尤其是 PR-AUC）。它不是調校後 GBDT
line 在頭條 AUC 上的 drop-in 替代品、推論慢得多，且在此任務中並未降低特徵工程
負擔。這正是本次比較的用意：一份嚴謹、多軸的解讀，而非一句頭條宣稱。
