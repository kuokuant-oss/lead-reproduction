...
## 2026-06-22 note: PI-requested 50/50 building split

M3.2a added a PI-requested design check where train and validation each use
about half of the buildings via a deterministic `building_id % 2` split
(725/724 buildings). This extends ADR 0001 because the split remains
building_id-based with zero train/validation building overlap. It does not
replace the canonical M3 80/20 `building_id % 5 == 4` validation line unless the
headline reporting protocol changes.
