...
def write_markdown(
    ...
    parts = [
        ...
        "## Setup",
        "",
        "- **Models.** TabPFN (`n_estimators=8`) and a tree ensemble "
        "(LightGBM, XGBoost, CatBoost, HistGradientBoosting; the `ensemble` "
        "score is what is reported).",
        "- **Label budget N.** 5,000 / 10,000 / 20,000 / 50,000 / 100,000 rows, "
        "50/50 stratified, seed 42, nested prefixes. Both models are fit on "
        "byte-identical rows at each N.",
        "- **Feature lines.** 17 and 137, run independently.",
        f"- **Holdout.** {summary['gates']['holdout_rows']:,} rows, "
        f"{summary['gates']['holdout_anomalies']:,} anomalies, buildings disjoint "
        "from the fit rows. Scored in full, then split by group.",
        "- **Metrics.** ROC-AUC and PR-AUC computed within each site and each "
        "meter type, not pooled and not reweighted.",
        "- **Layout.** One table per model; columns are N. The tree tables carry "
        "`FULL` on the right: the same tree ensemble trained on the entire "
        f"training set, pooled ROC {ref['17']['pooled_roc']:.4f} at 17 features "
        f"and {ref['137']['pooled_roc']:.4f} at 137 "
        "(`m3_feature_engineering_roc.png`). TabPFN has no such column; it was "
        "never run on the full training set.",
        "- **Draw.** One frozen context draw, seed 42. No repetition, so per-group "
        "context noise is unmeasured; row-sampling noise is measured for the thin "
        "sites in the last section.",
        ...
        "## Gates",
        "",
        "- Tree and TabPFN artifacts must carry identical `raw_index` and "
        "identical labels per cell; a cell failing this never reaches the metric "
        "code.",
        "- `meter` is recovered positionally from the frozen M3 frame; "
        "`building_id` and `site_id` read back at those positions must match what "
        "the prediction artifacts stored.",
        "- The two full-data M3 artifacts store `validation_raw_index` ascending "
        "while their score and label arrays are in canonical scoring order. They "
        "are consumed positionally here and never through that index, which would "
        "collapse the 137-feature line to ROC 0.4933.",
        "",
    ]
