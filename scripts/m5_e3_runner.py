...
def main() -> int:
    ...
    ap.add_argument("--target-repeats", type=int, required=True)
    ...
    if (
        args.target_repeats
        not in proto["supplementary_decisions"]["3_repeat_batches"]["batches"]
    ):
        raise SystemExit("target repeats must be a frozen batch size")
    ...
    if fit_marker.exists() and len(done) < args.target_repeats:
    ...
    for r in range(len(done), args.target_repeats):
        ...
        atomic_json(
            repeats_dir / f"repeat_{r:03d}.json",
            {
                "cell": args.cell,
                "repeat": r,
                "mode": "same_process",
        ...
        print(f"  repeat {r + 1}/{args.target_repeats} ({secs:,.1f}s)", flush=True)

    atomic_json(
        root / "CELL_COMPLETE.json",
        {
            "cell": args.cell,
            "repeats": args.target_repeats,
            "process_uuid": process_uuid,
            "state_sha256": state_sha,
            "completed": time.time(),
        },
    ...
    atomic_json(
        root / "heartbeat.json",
        {
            "cell": args.cell,
            "status": "completed",
            "completed_repeats": args.target_repeats,
            "target_repeats": args.target_repeats,
            "process_uuid": process_uuid,
            "timestamp": time.time(),
        },
    ...
    print(
        f"cell {args.cell} complete: {args.target_repeats} same-process repeats",
        flush=True,
    )
    return 0
