param(
    [string]$TaskName = "CodexTabPFNColabHeadRecoverySupervisor"
)

$ErrorActionPreference = "Stop"
$repoRoot = Split-Path -Parent $PSScriptRoot
$launcher = Join-Path $PSScriptRoot "launch_m5_colab_head_recovery_supervisor.ps1"
$lockPath = Join-Path $repoRoot "data\processed\m5_tabpfn_colab_head_recovery_supervisor.lock"
...
if (-not (Test-Path -LiteralPath $launcher -PathType Leaf)) {
    throw "Missing committed head-only launcher: $launcher"
}
...
throw "Scheduled head recovery supervisor did not acquire its singleton lock"
